...
void loadtd(TODO** t_head, TODO** t_tail) {
	...
	if (file != NULL) {
		...
		for (i = 0; i < count; i++) {
			TODO* t_new;
			t_new = (TODO*)malloc(sizeof(TODO));
			if (t_new == NULL) return;
			fscanf_s(file, "%d %d %d %d %d %d", &t_new->year, &t_new->month, &t_new->day, &t_new->hour, &t_new->min, &t_new->clear);
			fseek(file, 2, SEEK_CUR);
			fscanf_s(file, "%[^\n]", t_new->title, sizeof(t_new->title));
			fseek(file, 2, SEEK_CUR);
			fscanf_s(file, "%[^\n]", t_new->info, sizeof(t_new->info));
			t_new->next = NULL;
			*t_temp = t_new;
			*t_tail = *t_temp;
			t_temp = &(*t_temp)->next;
